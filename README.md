...
Holon treats identity as explicit, not inferred.

### Intent ID (includes model routing)

An intent is generated by a specific model, and multiple models may generate competing intent proposals concurrently:

- `I-{timestamp}-{short_hash}-{agent_family}-{model_id}`
- Example: `I-1738901234-a3f2-gemini-gemini_flash`

This allows the system to compare how different models decompose the same parent goal.

### Plan ID (includes model routing)

A plan is a versioned variant tied to an intent **and** the model tier used to produce it:

- `P-{intent_id}-v{variant}-{agent_family}-{model_id}`
- Example: `P-I-1738901234-a3f2-gemini-gemini_flash-v2-claude-sonnet`

Note: The intent's model is embedded in `{intent_id}`, and the planning model is specified separately.

### Branch naming

- Intent branch: `intent/{intent_id}`
- Plan branch (recommended): `intent/{intent_id}/plan/{plan_id}`
